...
$Log: include:GUSI_P.h,v $
Revision 1.2  1999/04/23  08:56:47  peterg
Automatic checkin:
...
public:
	...
	void		InitConsole();
	int		Install(Socket * sock, int from = -1, int start = 0);
	int		Remove(int fd);
...
struct GUSIConfiguration {
	...
	unsigned 		noChdir 		: 1;	// Set current directory without chdir()
	unsigned 		accurStat	: 1;	// Return # of subdirectories + 2 in st_nlink
	unsigned 		filler1		: 2;	// Used to be daemon bits, leave 'em alone for now
	unsigned 		hasConsole	: 1;	// Do we have our own console ?
	unsigned 		filler2		: 3;
...
extern const GUSIConfiguration	GUSIConfig;
