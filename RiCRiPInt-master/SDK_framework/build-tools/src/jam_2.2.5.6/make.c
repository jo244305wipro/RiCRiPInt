...
static void make0(TARGET *, int, timestamp_t, int, COUNTS *, int);
...
int	anyhow;
{
	...
	for( i = 0; i < n_targets; i++ )
	{
	    ...
	    make0( t, T_BIND_UNBOUND, timestamp, 0, counts, anyhow );
	}
...
/*
 * make0_includes() - collect includes
 */

static void
make0_includes( t, depends, addedsomemore )
TARGET	*t;
TARGETS	*depends;
int	*addedsomemore;
{
	...
	for( c = t->deps[T_DEPS_INCLUDES]; c; c = c->next ) {
	    /* Skip this target if the parent already depends on it */
	    if( c->target->progress == T_MAKE_ONSTACK ) continue;

	    /* Add target to the depends list */
	    targetentry( depends, c->target );
	    *addedsomemore = 1;

	    /* Recurse on its INCLUDES */
	    make0_includes( c->target, depends, addedsomemore );
	}
}
...
timestamp_t	ptime;		/* parent target's timestamp */
...
int	anyhow;		/* forcibly touch all (real) targets */
{
	...
	int	i;
	int	fate, hfate;
	timestamp_t	last, leaf, hlast, hleaf;
	char	*flag = "";
	...
	if( t->binding == T_BIND_UNBOUND && !( t->flags & T_FLAG_NOTFILE ) )
	{
	    ...
	    t->binding = TIME_NONZERO( t->time ) ? T_BIND_EXISTS : T_BIND_MISSING;
	}

	/* If temp file doesn't exist, use parent */

	if( t->binding == T_BIND_MISSING && t->flags & T_FLAG_TEMP && TIME_NONZERO( ptime ) )
	{
	    t->time = ptime;
	    t->binding = TIME_NONZERO( t->time ) ? T_BIND_PARENTS : T_BIND_MISSING;
	}
...
# ifdef USE_WIN32_API
		printf( "time\t--\t%s%s: %x/%x\n",
			spaces( depth ), t->name, t->time.HighPart, t->time.LowPart );
# else
		printf( "time\t--\t%s%s: %s",
...
# endif
	...
	/* Inherit the INCLUDES of our dependents */

	{
	    int addedsomemore;
	    TARGETS *tail = t->deps[ T_DEPS_DEPENDS ];

	    do {
		/*
		 * Recurse on the as-yet unprocessed dependents.  We
		 * need to do this now, rather than later, so that we
		 * can discover included header files.  Continue where
		...
		/*
		 * Go through the new DEPENDS, recursively adding their
		 * INCLUDES (which may now have been extended) to these
		 * DEPENDS, avoid repeats through the use of marks.
		 * (We must remove the marks before we continue.)
		 */
		addedsomemore = 0;
		for( c = t->deps[ T_DEPS_DEPENDS ]; c; c = c->next )
		    c->target->progress = T_MAKE_ONSTACK;

		for( c = first; c; c = c->next )
		    make0_includes( c->target, t->deps[ T_DEPS_DEPENDS ], &addedsomemore );

		for( c = t->deps[ T_DEPS_DEPENDS ]; c; c = c->next )
		    c->target->progress = T_MAKE_INIT;

		if( tail ) tail = tail->next;
	    } while( addedsomemore );
	}

	/* Now recurse on the non-includable depends */

	for( c = t->deps[ T_DEPS_NIDEPENDS ]; c; c = c->next )
	    make0( c->target, t->binding, t->time, depth + 1, counts, anyhow );

	/* Step 3a: find the most recent of our dependents */

	for( i = T_DEPS_DEPENDS; i <= T_DEPS_NIDEPENDS; i++ )
	    for( c = t->deps[ i ]; c; c = c->next )
	{
	    leaf = TIME_NEWEST( leaf, c->target->leaf );
	    leaf = TIME_NEWEST( leaf, c->target->hleaf );
	    ...
	    last = TIME_NEWEST( last, c->target->time );
	    last = TIME_NEWEST( last, c->target->htime );
	...
	*/
	...
	else if( t->binding == T_BIND_EXISTS && TIME_NEWER( last, t->time ) )
	{
	    fate = T_FATE_OUTDATED;
	}
	else if( t->binding == T_BIND_PARENTS && TIME_NEWER( last, t->time ) )
	{
	    fate = T_FATE_OUTDATED;
	}
	...
	else if( t->binding == T_BIND_EXISTS && TIME_NONZERO( ptime ) && TIME_NEWER( t->time, ptime ) )
	{
	    fate = T_FATE_NEWER;
	}
	...
	t->time = TIME_NEWEST( t->time, last );
	t->leaf = TIME_NONZERO( leaf ) ? leaf : t->time ;
	t->fate = fate;
	...
	for( c = t->deps[ T_DEPS_INCLUDES ]; c; c = c->next )
	{
	    ...
	    hlast = TIME_NEWEST( hlast, c->target->time );
	    hlast = TIME_NEWEST( hlast, c->target->htime );
	    hleaf = TIME_NEWEST( hleaf, c->target->leaf );
	    hleaf = TIME_NEWEST( hleaf, c->target->hleaf );
	...
	t->htime = hlast;
	t->hleaf = TIME_NONZERO( hleaf ) ? hleaf : t->htime;
	t->hfate = hfate;
