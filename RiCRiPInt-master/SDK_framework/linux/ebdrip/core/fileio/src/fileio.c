...
#include "gps/host.h"		/*VENKAT ADDED*/
...
extern gps_hostbuf_p host_inBuff;
/*extern gps_hostbuf_t host_inBuff;*/
/*VENKAT ENDS*/
...
/* flptr must be an input file */
{
  ...
  /*VENKAT STARTS */
  if(strcmp(flptr->clist, "/input") != 0)
	{
	      input_size = theIBufferSize(flptr);
	}
  else
	{
	      if(strcmp(flptr->clist, "/input") == 0)
/*		input_size = gpsSinAvailEof(host_inBuff,&nBytesToRead); */
		gpsSinAvailEof(host_inBuff,&input_size);
	}
    /*VENKAT ENDS*/
