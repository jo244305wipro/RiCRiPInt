...
int OIL_Init(void (**apfn_funcs[])())
{
  ...
  if(retvalGPSOpen = GPS_Open(gps_client, GWID_Event_Handler, attr, (void *)(&shdm_addr)) < 0)*/

 /* if(retvalGPSOpen=gpsOpen(gps_client, GWID_Event_Handler, &attr, (void *)(&shdm_addr))<0)
  {
       printf("*** GPSOpen() returns %d.\n", retvalGPSOpen);
       return -1;
  }*/
#endif
...
#else
  if(find_PDLType(&eOIL_PDL)) /*VENKAT ADDED*/
#endif
  {
    Stream_Reset();
...
//VENKAT STARTS
//The below API definiton Needs to be replace at the begining.
...
// GPS Supports many other PDL Types mentioned in "Interp.h". As of now, other PDL types are being redirected to PCL5c.
	...
	ePDLType = OIL_PDL_PCL5c; //Changed pdl_id to 10(default is 9) in ".gpsconf" and Hard coding to OIL_PDL_PCL5c to check.
