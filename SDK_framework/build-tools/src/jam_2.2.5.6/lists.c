 ...
 *	list_tack() - tack a value onto the end of a list
 *	list_dup() - tack a "copy" of a value onto the end of a list
...
void	*string;
{
	return list_tack( head, STRING, string );
}
...
void	*value;
{
	...
	{
	    l = freelist;
	    if( l->type == STRING || l->type == EXPRESSION )
		freestr( l->value );
...
/*
 * list_tack() - tack a "copy" of a value onto the end of a list
 */
...
LIST	*l;
{
	switch( l->type )
	{
	case STRING: case EXPRESSION:
	    return list_new( head, copystr( l->value ) );
	default:
	    return list_tack( head, l->type, l->value );
	}
}
...
LIST	*l;
{
	for( ; l; l = list_next( l ) )
	{
		...
		break;
	    case VARREF:
		list_print( ((VARIABLE*)l->value)->value );
		break;
	    }
	}
}
...
int	i;
{
	return (lol && i < lol->count) ? lol->list[i] : 0;
}
