...
ACTIONS	*a0;
{
	CMD *cmds = 0;
	CMD *cmdlast = 0;
	LIST *shell = var_collapse( L0, var_get( "JAMSHELL" ) );	/* shell is per-target */
...
{
	SETTINGS *settings = 0;
	LIST *collapsed = var_collapse( L0, vars );

	for( vars = collapsed; vars; vars = list_next( vars ) )
	{
	    LIST *ll = var_collapse( L0, var_get( vars->value ) );
	    LIST *l;
	    LIST *nl = 0;
	    ...
	    settings = addsettings( settings, 0, vars->value, nl );
